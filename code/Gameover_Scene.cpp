...
namespace flip
{
    ...
    bool Gameover_Scene::initialize ()
    {
    ...
    void Gameover_Scene::handle (basics::Event & event)
    {
        if (state == READY)
        {
            switch (event.id)
    ...
    void Gameover_Scene::update (float time)
    {
        if (!suspended) if (state == LOADING)
        {
            ...
            if (context)
            {
                ...
                gameover_texture = Texture_2D::create (ID(gameover_texture), context, gameover_path);      // Loads the gameover texture
                button_texture   = Texture_2D::create (ID(button_texture),   context, button_path);        // Loads the button texture

                context->add(gameover_texture);
    ...
    void Gameover_Scene::render (Graphics_Context::Accessor & context)
    {
        if (!suspended)
        {
            ...
            if (canvas)
            {
                ...
                if (state == READY)
                {
                    ...
                    canvas->fill_rectangle ({ 0.f, 0.f }, { (float) canvas_width, (float) canvas_height }, gameover_texture.get (), Anchor::BOTTOM | Anchor::LEFT);
    ...
    void Gameover_Scene::configure_options ()
    {
    ...
    int Gameover_Scene::option_at (const Point2f & point)
    {
        for (int index = 0; index < number_of_options; ++index)
        {
